...
| 映像表示 | 設定タブとコントローラーの見切れを修正。大きな表示サイズでもモニター内に収め、解像度変更時の配置を固定。小さい映像は縦横比を保った拡大表示も選択可能。確認ダイアログを削除。 |
...
| 接続・起動 | 同じexeから複数画面を開く際に設定枠を自動選択。指定COMポート名でのボーレート適用、カメラ無効時とスクリプト読み込み失敗時の処理を修正。 |
...
同じexeをもう一度起動すると、空いている設定枠を自動で選んで別ウィンドウを開きます。各枠の設定は保存され、次回も再利用されます。新しい枠ではカメラとCOMポートを選んでください。名前を指定したい場合は`--profile 名前`も使用できます。
...
「表示サイズ」は映像の基準サイズです。起動時にモニターへ収まる表示領域を確保し、解像度を変更してもウィンドウ・操作欄・ログ欄の位置とサイズを維持します。「表示領域に合わせて拡大」をONにすると、小さい映像も縦横比を保って領域いっぱいに表示します。保存される指定値とキャプチャ・画像認識の解像度は変更しません。
